        ...
        var BIG_DURATION = 1200;     // 20 seconds at 60fps
        var BIG_SCALE = 2.3;
        ...
        function processLiveEvent(data) {
            ...
            if (data.type === 'connected') {
                ...
                if (giftName.includes('perfume')) {
                    ...
                    var oldPick = getPickaxeForGifts(gUser);
                    ...
                    var newPick = getPickaxeForGifts(gUser);
                    if (newPick && oldPick && newPick.name !== oldPick.name) {
                        showPickaxeUpgradeOverlay(gUser, gAvatar, oldPick, newPick);
                    }
                }
        ...
        function showPickaxeUpgradeOverlay(userName, userAvatar, oldPick, newPick) {
            ...
            if (newPick && newPick.color) {
                aura.style.background = 'radial-gradient(circle, ' + newPick.color + '44 0%, rgba(0,0,0,0) 70%)';
            }
            ...
            if (newPick && newPick.color) {
                avatarImg.style.boxShadow = '0 0 25px ' + newPick.color + ', inset 0 0 10px rgba(0,0,0,0.5)';
                avatarImg.style.borderColor = newPick.color;
            }
            ...
            if (newPick.color) {
                titleText.style.color = newPick.color;
            }
            ...
            // Play second sound when the new pickaxe pops in (600ms)
            setTimeout(function() {
                if (_audioStarted) {
                    playPowerUp();
                }
                ...
                for (var i = 0; i < 20; i++) {
                    createUpgradeParticle(wrapper, newPick.color || '#ffd700');
                }
            }, 600);
