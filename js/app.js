var persistentScores = {};
        ...
        function getTargetUsers() {
            var selected = [];
            document.querySelectorAll('.player-checkbox:checked').forEach(function(cb) {
                selected.push(cb.value);
            });
            if (selected.length === 0 && pick) {
                selected.push(pick.userName || '@player');
            }
            return selected;
        }

        function findPickForTarget(u) {
            if (!u) return null;
            var cleanU = u.replace(/^@/, '').toLowerCase();
            if (pick) {
                var cleanPickUser = (pick.userName || '').replace(/^@/, '').toLowerCase();
                var cleanOwner = (ownerName || '').replace(/^@/, '').toLowerCase();
                if (cleanU === cleanPickUser || cleanU === 'player' || cleanU === cleanOwner) {
                    return pick;
                }
            }
            return userPicks.find(function(x) {
                var cleanPUser = (x.userName || '').replace(/^@/, '').toLowerCase();
                return cleanPUser === cleanU;
            });
        }

        function addCoinToSelected(amount) {
            getTargetUsers().forEach(function(u) {
                var data = persistentScores[u];
                if (!data) return;
                
                // Adiciona moedas (coins)
                data.coins = (data.coins || 0) + amount;
                
                // Usa a função existente para exibir o banner e tocar o som!
                if (typeof showCoinBanner === 'function') {
                    showCoinBanner(u, amount);
                    if (_audioStarted && typeof coinSound !== 'undefined') coinSound.play().catch(function(e){});
                } else {
                    // Fallback se por algum motivo não achar
                    var pickObj = userPicks.find(p => p.userName === u);
                    var x = pickObj ? pickObj.x : canvas.width / 2;
                    var y = pickObj ? pickObj.y : camY + 100;
                    spawnText(x, y - 30, '+' + amount + ' 🪙', '#ffdd00');
                    if (typeof playPowerUp === 'function') playPowerUp();
                }
            });
        }

        function updatePlayerManagementPanel() {
            var panel = document.getElementById('playerList');
            if (!panel) return;
            
            // Guardar o estado dos checkboxes marcados antes de limpar
            var checkedUsers = new Set();
            document.querySelectorAll('.player-checkbox:checked').forEach(function(cb) {
                checkedUsers.add(cb.value);
            });
            
            var users = Object.keys(persistentScores).filter(function(name) {
                var lower = name.toLowerCase();
                return lower !== 'esqueleto wither' && lower !== '@player';
            });
            
            // Sort by status (Green > Yellow > Red) and then by score descending
            users.sort(function(a, b) {
                var inGameA = userPicks.some(function(up) { return up.userName === a; });
                var lastActA = lastActivity[a] || 0;
                var isWatchingA = !inGameA && (Date.now() - lastActA < 5 * 60 * 1000);
                var statusA = inGameA ? 0 : (isWatchingA ? 1 : 2);

                var inGameB = userPicks.some(function(up) { return up.userName === b; });
                var lastActB = lastActivity[b] || 0;
                var isWatchingB = !inGameB && (Date.now() - lastActB < 5 * 60 * 1000);
                var statusB = inGameB ? 0 : (isWatchingB ? 1 : 2);

                if (statusA !== statusB) {
                    return statusA - statusB;
                }
                
                return persistentScores[b].score - persistentScores[a].score;
            });
            ...
            users.forEach(function(u) {
                var data = persistentScores[u];
                if (!data.joinTime) data.joinTime = Date.now();
                
                // Status do jogador
                var inGame = userPicks.some(function(up) { return up.userName === u; });
                var lastAct = lastActivity[u] || 0;
                var isWatching = !inGame && (Date.now() - lastAct < 5 * 60 * 1000); // 5 minutos de inatividade
                
                var statusColor = '#ff4444'; // Vermelho (Saiu)
                if (inGame) statusColor = '#00ff88'; // Verde (Na partida)
                else if (isWatching) statusColor = '#ffbb00'; // Amarelo (Assistindo)
                ...
                dot.style.boxShadow = '0 0 5px ' + statusColor;
                ...
                cb.type = 'checkbox';
                cb.value = u;
                cb.className = 'player-checkbox';
                cb.style.cursor = 'pointer';
                if (checkedUsers.has(u)) cb.checked = true;
                item.appendChild(cb);
                ...
                avatar.onerror = function() { this.src = 'https://www.gravatar.com/avatar/00000000000000000000000000000000?d=mp&f=y'; };
                item.appendChild(avatar);
                
                var info = document.createElement('div');
                info.style.flex = '1';
                
                var playTime = Math.floor((Date.now() - data.joinTime) / 1000); // in seconds
                var timeStr = playTime < 60 ? playTime + 's' : Math.floor(playTime / 60) + 'm';
                
                info.innerHTML = '<div style="font-weight:bold; color:' + (data.color || '#fff') + ';">' + u + '</div>' +
                                 '<div style="color:#aaa; font-size:10px;">Score: ' + data.score + ' | Time: ' + timeStr + '</div>' +
                                 '<div style="display:flex; gap:6px; margin-top:2px; align-items:center;">' +
                                 (data.isChicken ? '<span style="font-size:10px;">🐔</span>' : '') +
                                 (data.isSteve ? '<span style="font-size:10px;">💎</span>' : '') +
                                 (data.isHomer ? '<span style="font-size:10px;">🍩</span>' : '') +
                                 (data.coins ? '<span style="color:#ffdd00; font-size:10px; font-weight:bold;">🪙 ' + data.coins + '</span>' : '') +
                                 (data.giftsValue ? '<span style="color:#ff4488; font-size:10px; font-weight:bold;">🎁 ' + data.giftsValue + '</span>' : '') +
                                 '</div>';
        ...
        setInterval(updatePlayerManagementPanel, 2000);
        ...
        function cycleBgVideo() {
            ...
            if (currentBgIdx >= bgVideos.length) {
                ...
                if (wrapper) wrapper.style.background = '#0d0d2b';
                spawnText(canvas.width / 2, camY + canvas.height / 2, 'FUNDO: DINÂMICO', '#ffffff');
            } else {
                bgVideoActive = true;
                vid.src = bgVideos[currentBgIdx];
                
                // Force styling to ensure it sits behind the game canvas properly
        ...
        var TILE = window.IS_TITLE_MODE ? Math.max(12, Math.min(60, Math.floor(window.innerWidth / 40), Math.floor(window.innerHeight / 24))) : 72;
        ...
        var BTEX = {};
        var BGLOW_TEX = {};
        var B3D_TEX = {}; // Cache for pre-rendered 3D blocks
        var CREEPER_TEX = null;
        var BG_TEX = {};
        
        (function() {
            var bgFiles = ['mc_cloud_1', 'mc_cloud_2', 'mc_cloud_3', 'mc_cloud_4', 'mc_balloon', 'mc_rainbow', 'mc_planet', 'mc_moon'];
            bgFiles.forEach(function(k) {
                var img = new Image();
                img.src = 'bg_assets/' + k + '.png';
                BG_TEX[k] = img;
            });
        })();
        ...
        (function () {
            Object.keys(BDEF).forEach(function (k) {
                ...
                img.onload = function() {
                    var w = TILE;
                    var h = TILE;
                    var d = Math.floor(TILE * 0.3); // Depth of 3D effect
                    ...
                    cx.save();
                    cx.transform(d/w, -d/w, 0, 1, w, d);
                    cx.drawImage(img, 0, 0, img.naturalWidth, img.naturalHeight, 0, 0, w, h);
                    cx.fillStyle = 'rgba(0,0,0,0.5)'; // Darken right
                    cx.fillRect(0, 0, w, h);
                    cx.restore();
                ...
                var glowKeys = ['copper_ore', 'diamond_ore', 'emerald_ore', 'gold_ore', 'iron_ore', 'lapis_ore', 'redstone_ore', 'tnt', 'mega_tnt', 'heart_tnt', 'creeper'];
                var imgStr = BDEF[k].img || '';
                var hasGlow = glowKeys.some(function(gk) { return imgStr.indexOf(gk) !== -1; });
                if (hasGlow) {
                    var g_img = new Image();
                    g_img.src = BDEF[k].img.replace('block/', 'block_glows/').replace('.png', '_glow.png');
                    BGLOW_TEX[k] = g_img;
                }
            });
        ...
        function updateTNTBlocks() {
            ...
            for (var i = tntBlocks.length - 1; i >= 0; i--) {
                var b = tntBlocks[i];
                var hs = b.nuke ? TILE * 0.3 : (b.mega ? TILE * 0.9 : (b.creeper ? TILE : TILE * 0.5));
                b.blinkTimer++;
                b.vy += TNT_GRAVITY;
                // TNT floats in water
                ...
                var onScreen = b.y > camY - 200 && b.y < camY + canvas.height + 200;
        ...
        function detonateTNTBlock(b) {
            ...
            var radius = b.nuke ? 9 : (b.mega ? MEGA_TNT_RADIUS : (b.creeper ? CREEPER_RADIUS : (b.heart ? HEART_TNT_RADIUS : TNT_RADIUS)));
        ...
        // Creepers are now handled in tntBlocks for standardization.
        ...
        function activateCreeper(tx, ty, userName) {
            if (tntBlocks.length > 150) return;
            var sx = tx !== undefined ? tx : pick.x;
            var sy = ty !== undefined ? ty : pick.y;
            ...
            tntBlocks.push({
                ...
                blinkTimer: -(_tntSpawnCount % 6) * 20 - Math.floor(Math.random() * 15),
                fuse: 160, // Padronizado com Mega TNT
                creeper: true,
                userName: userName
            });
        }
        ...
        function getPickaxeLevel(p) {
            if (!p || !p.pickaxe) return 0;
            var name = p.pickaxe.name;
            var idx = PICKAXES.findIndex(function (px) { return px.name === name; });
            if (idx === -1) {
                if (name === 'Mario' || name === 'Homer') return 6;
                return 0;
            }
            return idx;
        }
        ...
        function showCoinBanner(userName, coins) {
            ...
            // Banner no estilo de inscrição
            var overlay = document.getElementById('announceOverlay');
            if (!overlay) return;

            var cfg = { icon: '🪙', title: 'GANHOU COIN!', color: '#ffd700', dur: 150 };
            var avatar = '';
            if (uPick && uPick.avatarUrl) { avatar = uPick.avatarUrl; }
            if (!avatar && persistentScores[userName] && persistentScores[userName].avatar) {
                avatar = persistentScores[userName].avatar;
            }
            ...
            card.innerHTML =
                ...
                '<div class="announce-name">' + userName + '</div>' +
                '<div class="announce-pts" style="color:' + cfg.color + '">+' + coins + ' COIN' + (coins > 1 ? 'S' : '') + '</div>';
            ...
            if (_audioStarted) {
                try { sfxSkillActivate(); } catch (e) { }
            }
        }

        function addCycleScore(userName, pts, type) {
            var lower = userName.toLowerCase();
            if (!userName || lower === 'esqueleto wither' || lower === '@player') return;
        ...
        function triggerCycleEnd(cycle) {
            ...
            setTimeout(function () {
                var animatingCount = top3.length;
                top3.forEach(function (p, i) {
                    ...
                    function processNext() {
                        if (qIdx >= queue.length) {
                            ...
                            animatingCount--;
                            if (animatingCount <= 0) {
                                // Reduz o tempo restante para sumir rápido (3 segundos)
                                if (_cycleOverlay && _cycleOverlay.timer > 180) {
                                    _cycleOverlay.timer = 180;
                                }
                            }
                            return;
                        }
        ...
        var SWORD_DURATION = 1200; // 20 seconds
        ...
        var bigCooldown = 1200;     // 20 seconds at 60fps
        var ballCooldown = 1800;      // 30 seconds at 60fps
        var BALL_DURATION = 1800;     // 30 seconds at 60fps
        var activeBalls = [];
        var creeperCooldown = 900; // 15 seconds at 60fps

        var BIG_DURATION = 1200;     // 20 seconds at 60 seconds at 60fps
        var BIG_SCALE = 2.3;
        ...
        function restart() {
            ...
            worldMap = {}; explosions = []; tntBlocks = []; items = []; zombies = []; bats = []; spiders = []; spiderWebs = []; fallingBlocks = []; bonusBalls = []; collectBoxes = []; activeBalls = []; clearedZones = {}; clearingShapes = {}; crystalDecoList = []; torchList = []; clearingCrystals = {}; megaClearedZones = {};
            miniClearedZones = {};
            ...
            document.getElementById('scoreDisplay').innerHTML = '&#9935; ' + displayScore;
            ensureRows(0, VIS + 4);
            spawnPick();
        ...
        function activateDoublePick() {
            ...
            if (pick) extraPicks.push({
                ...
                swordTimer: pick.swordTimer || 0,
                bigTimer: pick.bigTimer || 0
            });
            if (_audioStarted) sfxSkillActivate();
            updateDoublePickHUD();
        }
        ...
        function activateBall(tx, ty, userName, avatarUrl) {
            userName = (userName || '').replace(/^@@+/, '@');
            var cdKey = userName + '_ball';
            if (skillCooldowns[cdKey] && skillCooldowns[cdKey] > 0) return;
            var p = null;
            ...
            if (!p) p = pick;
            if (!p) return;
            skillCooldowns[cdKey] = ballCooldown;
            var targetX = tx !== undefined ? tx : p.x;
            var targetY = ty !== undefined ? ty : p.y;
            var strongestPickaxe = getStrongestPickaxe();
            activeBalls.push({
                x: targetX,
                y: targetY,
                ...
                imageIndex: BALL_TEX.length ? Math.floor(Math.random() * BALL_TEX.length) : 0,
                ownerName: userName,
                ownerAvatarUrl: avatarUrl || p.userAvatarUrl || '',
                ownerColor: p.color || '#ffdd44',
                pickaxe: strongestPickaxe,
                damage: Math.max(1, Math.ceil(strongestPickaxe ? strongestPickaxe.damage : 1))
            });
            spawnText(targetX, targetY - 40, '⚽ BALL POWER!', '#ffdd44');
            if (_audioStarted) sfxSkillActivate();
        }
        ...
        function updateActiveBalls() {
            for (var i = activeBalls.length - 1; i >= 0; i--) {
                ...
                ball.timer--;
                ball.stuck = false;
                var hitAny = applyPhysics(ball, ball.r, { skipFloorReset: true });
                if (hitAny) {
                    shakeAmt = Math.max(shakeAmt, 6);
                }
                if (ball.y > camY + canvas.height + 800 || ball.timer <= 0) {
        ...
        function drawActiveBalls() {
            for (var i = 0; i < activeBalls.length; i++) {
                var ball = activeBalls[i];
                if (ball.y < camY - 60 || ball.y > camY + canvas.height + 60) continue;

                ctx.save();
                ctx.translate(ball.x, ball.y);
                if (ball.ang) ctx.rotate(ball.ang);
                var img = BALL_TEX.length ? BALL_TEX[ball.imageIndex % BALL_TEX.length] : null;
                if (img && img.complete && img.naturalWidth > 0) {
                    ctx.drawImage(img, -ball.r, -ball.r, ball.r * 2, ball.r * 2);
                } else {
                    ctx.beginPath();
                    ctx.arc(0, 0, ball.r, 0, Math.PI * 2);
                    ctx.fillStyle = ball.ownerColor || '#ffdd44';
                    ctx.fill();
                    ctx.lineWidth = 3;
                ...
                if (ball.ownerName) {
                    ctx.save();
                    var ownerText = ball.ownerName;
                    var avImg = ball.ownerAvatarUrl ? avatarCache[ball.ownerAvatarUrl] : null;
                    var textY = ball.y - ball.r - 18;
                    var textX = ball.x + 5;
                    if (avImg && avImg.complete && avImg.naturalWidth > 0) {
                        var avSize = 24;
                        ctx.save();
                        ctx.beginPath();
                        ctx.arc(ball.x - 5 - avSize / 2, textY, avSize / 2, 0, Math.PI * 2);
                        ctx.clip();
                        ctx.drawImage(avImg, ball.x - 5 - avSize, textY - avSize / 2, avSize, avSize);
                        ctx.restore();
                        ctx.strokeStyle = '#fff';
                        ctx.lineWidth = 2;
                        ctx.beginPath();
                    ...
                    ctx.lineWidth = 4;
                    ctx.lineJoin = 'round';
                    ctx.strokeText(ownerText, textX, textY);
                    ctx.fillStyle = '#ffffff';
                    if (SHADOW_ENABLED) {
                        ctx.shadowBlur = 4;
                        ctx.shadowColor = '#000';
                    }
                    ctx.fillText(ownerText, textX, textY);
                    ctx.restore();
                }
            }
        }

        function getStrongestPickaxe() {
            var best = currentPickaxe || PICKAXES[PICKAXES.length - 1] || { damage: 1 };
            if (!best || best.damage === undefined) best = { damage: 1 };
            if (MARIO_PICKAXE && MARIO_PICKAXE.damage > best.damage) best = MARIO_PICKAXE;
            if (HOMER_PICKAXE && HOMER_PICKAXE.damage > best.damage) best = HOMER_PICKAXE;
            if (pick && pick.pickaxe && pick.pickaxe.damage > best.damage) best = pick.pickaxe;
            for (var ui = 0; ui < userPicks.length; ui++) {
                var up = userPicks[ui];
                if (up.pickaxe && up.pickaxe.damage > best.damage) best = up.pickaxe;
            }
            return best;
        }
        ...
        function checkBallEnemyCollisions() {
            for (var bi = activeBalls.length - 1; bi >= 0; bi--) {
                ...
                for (var ei = 0; ei < enemies.length; ei++) {
                    var e = enemies[ei];
                    if (!e || e.state === 'dead' || e._bhCaptured) continue;
                    var isZombie = zombies.indexOf(e) !== -1;
                    var isBat = bats.indexOf(e) !== -1;
                    var isSpider = spiders.indexOf(e) !== -1;
                    var isWither = witherSkeletons.indexOf(e) !== -1;
                    var eRadius = 22;
                    if (isZombie) {
                        eRadius = ZOMBIE_SIZE * 0.4 * (e.isZBoss ? ZBOSS_SCALE : 1);
                    } else if (isBat) {
                        eRadius = BAT_SIZE * 0.45;
                        if (e.isBlaze) eRadius *= 3.5;
                        else if (e.isGhast) eRadius *= (e.isMother ? 3.0 : 1.2);
                        else if (e.isQueen) eRadius *= QUEEN_SCALE;
                        else if (e.isMother) eRadius *= MOTHER_SCALE;
                        else eRadius *= 1.3;
                    } else if (isSpider) {
                        eRadius = SPIDER_SIZE * 0.35 * (e.isMother ? SPIDER_MOTHER_SCALE : 1);
                    } else if (isWither) {
                        eRadius = 22;
                    }
                    ...
                    var relVx = ball.vx - (e.vx || 0);
                    var relVy = ball.vy - (e.vy || 0);
                    var dot = relVx * (-nx) + relVy * (-ny);
                    if (dot > 0) {
                        var pushForce = dot * 2.0;
                        ball.vx += nx * pushForce * 0.25;
                        ball.vy += ny * pushForce * 0.25;
                        if (e.vx !== undefined) e.vx -= nx * pushForce * 1.0;
                        if (e.vy !== undefined) e.vy -= ny * pushForce * 1.0;
                    }
                    ball.vy = Math.min(ball.vy, -3.5);
                    ball.spin += (Math.random() - 0.5) * 0.12;

                    var damage = Math.max(1, Math.ceil(ball.damage || (ball.pickaxe && ball.pickaxe.damage) || 1));
                    e.hp -= damage;
                    spawnText(e.x, e.y - 40, '-' + damage + ' ⚽', '#ff8844');
                    registerComboHit(ball, e.x, e.y);
                    ...
                    if (e.hp <= 0) {
                        ...
                        if (isZombie) {
                            ...
                            sfxZombieDeath();
                            if (ball.ownerName) addCycleScore(ball.ownerName, e.pts || 5, 'kill');
                        } else if (isBat) {
                            e.state = 'dead'; e.anim = 0; e.deadTimer = 0;
                            sfxBatDeath();
                            if (ball.ownerName) addCycleScore(ball.ownerName, e.pts || 25, 'kill');
                        } else if (isSpider) {
                            e.state = 'dead'; e.animTimer = 0; e.deadTimer = 0;
                            sfxSpiderDeath();
                            if (ball.ownerName) addCycleScore(ball.ownerName, e.pts || 25, 'kill');
                            ...
                            playWitherDeath();
                            if (typeof spawnParts === 'function') {
                                spawnParts(e.x, e.y, '#555555', 10);
                                spawnParts(e.x, e.y, '#aaaaaa', 10);
                            }
                            witherSkeletons.splice(witherSkeletons.indexOf(e), 1);
                            spawnText(e.x, e.y, '💀 DEAD', '#ffffff');
                            if (ball.ownerName) addCycleScore(ball.ownerName, e.pts || 50, 'kill');
                        } else {
                            e.state = 'dead';
                        }
                        break;
                        ...
                        if (isSpider) {
                            e.state = 'hurt'; e.hurtTimer = 12; e.animTimer = 0;
                        } else if (isBat) {
                            e.state = 'hurt';
                        } else if (isZombie) {
                            e.state = 'hurt'; e.hurtTimer = 12; e.animTimer = 0;
        ...
        function activateBig(userName) {
            ...
            var lvl = getPickaxeLevel(p);
            var duration = Math.floor(BIG_DURATION * (1 + lvl * 0.1));
        ...
        function applyPhysics(p, r, opts) {
            ...
            if (p.bigTimer > 0) r = Math.floor(r * BIG_SCALE * (1 + getPickaxeLevel(p) * 0.1));
        ...
        function spawnItem(x, y, blockType, source) {
            ...
            if (target && target.isCompanion && target.owner) {
                target = target.owner;
            }
            var isLoose = false;
            if (target && target.userName === 'Esqueleto Wither') {
                target = null;
                isLoose = true;
            }
            items.push({
        ...
        function updateItems() {
            for (var i = items.length - 1; i >= 0; i--) {
                ...
                if (it.isLoose) {
                    // Falls naturally and floats in water
                    it.vx *= 0.95;
                    it.vy += 0.3;
                    if (isInWater(it.x, it.y)) { it.vx *= 0.9; it.vy *= 0.8; it.vy -= 0.35; }
                    ...
                    // Check proximity of all active players
                    var allP = getAllActivePicks();
                    var pickedUp = false;
                    for (var pi = 0; pi < allP.length; pi++) {
                        var pp = allP[pi];
                        if (!pp) continue;
                        var dxLoose = pp.x - it.x, dyLoose = pp.y - it.y;
                        var distLoose = Math.sqrt(dxLoose * dxLoose + dyLoose * dyLoose);
                        if (distLoose < 45) {
                            it.owner = (pp.isCompanion && pp.owner) ? pp.owner : pp;
                            it.isLoose = false;
                            pickedUp = true;
                            break;
                        }
                ...
                if (dist < 45) {
                    ...
                    if (it.isSword) {
                        ...
                        // Exibe animação de espada encontrada no mapa
                        if (typeof showSwordFoundOverlay === 'function') {
                            var uName = target.userName || '@Player';
                            var uAvatar = target.userAvatarUrl || (persistentScores[uName] ? persistentScores[uName].avatar : '');
                            showSwordFoundOverlay(uName, uAvatar, type, swordDef ? swordDef.color : '#fff');
                        }
                    } else {
        ...
        function drawFallingBlocks() {
            for (var i = 0; i < fallingBlocks.length; i++) {
                ...
                ctx.globalAlpha = 1;
                // Block texture
                var tex3d = B3D_TEX[fb.t];
                if (tex3d) {
                    var depth = Math.floor(fb.size * 0.3);
                    // To center the 3D block around fb.x, fb.y, we need to know the bounds.
                    // Front face was at -half, -half.
                    // The tex3d encompasses Front + Top + Right.
                    // Its width is size + depth, height is size + depth.
                    // Front face is at (0, depth) in tex3d.
                    // So we draw tex3d at (-half, -half - depth)
                    var targetW = fb.size + depth;
                    var targetH = fb.size + depth;
                    ctx.drawImage(tex3d, -half, -half - depth, targetW, targetH);
                    
                    var gTex = BGLOW_TEX[fb.t];
                    if (gTex && gTex.complete && gTex.naturalWidth > 0) {
                        ctx.save();
                        ctx.globalCompositeOperation = 'lighter';
                        ctx.globalAlpha = 0.6 + Math.sin(Date.now() * 0.005 + fb.x) * 0.4;
                        ctx.drawImage(gTex, -half, -half, fb.size, fb.size);
                        ctx.restore();
                    }
                } else if (tex && tex.complete && tex.naturalWidth > 0) {
        ...
        function updateBonusBalls() {
            ...
            for (var i = bonusBalls.length - 1; i >= 0; i--) {
                ...
                if (bonusBalls[i] && (ball.y > camY + canvas.height + 800 || ball.y < camY - 800 || ball.x < -200 || ball.x > COLS * TILE + 200)) {
                    bonusBalls.splice(i, 1);
                }
            }
        }
        ...
        function update() {
            ...
            if (uiUpdateTick % 10 === 0) {
                ...
                // Remove old pending element if exists
                var oldPending = scoreDisplay.nextElementSibling;
                if (oldPending && oldPending.classList.contains('score-pending')) {
                    oldPending.remove();
                }
                
                // Add pending score if exists
                if (pending > 0) {
                    var pendingEl = document.createElement('span');
                    pendingEl.className = 'score-pending';
                    pendingEl.innerHTML = '+' + pending + '&#9889;';
                    scoreDisplay.parentNode.insertBefore(pendingEl, scoreDisplay.nextSibling);
                }
            }
            ...
            if (isBossFightActive || dragonActive) {
                ...
                if (false && remaining <= 0 && !dragonActive) { // Dragão desativado por enquanto
                    dragonActive = true;
            ...
            var _allBigPicks = [pick].concat(userPicks, extraPicks);
                ...
                if (atk.timer <= 0) {
                    ...
                    if (targets.length > 0) {
                        ...
                        if (bestTarget) {
                            ...
                            var dmgVal = Math.ceil((isZ ? 3 : 1) * (1 + getPickaxeLevel(player) * 0.1));
                    ...
                    if (megaClearedZones[mz] && !megaClearedZones[mz + '_gateOpen']) {
                        ...
                        if (gateType === 'zombie') {
                            ...
                            for (var gci = 0; gci < bats.length; gci++) {
                                if (bats[gci].isGhast && bats[gci].maxHp > 100 && bats[gci].zone === mz && bats[gci].state !== 'dead') aliveBosses++;
                            }
                        } else {
                            // Default for Queen
        ...
        function updateWeatherParticles() {
            ...
            if (zone !== _lastWeatherZone) {
                ...
                for (var ti = 0; ti < types.length; ti++) {
                    var dt = types[ti];
                    if (dt === 'clouds') { for (var i = 0; i < 4; i++) { var cw = 200 + Math.random() * 500; _bgElements.push({ type: 'cloud', x: Math.random() * W * 1.5 - W * 0.25, y: Math.random() * (H + 600), w: cw, speed: 0.1 + Math.random() * 0.25, life: 600 + Math.random() * 400, z: 4 + cw/1000, parallax: 0.1 + cw/2000 }); } }
                    if (dt === 'birds') { for (var i = 0; i < 4; i++) _bgElements.push({ type: 'bird', x: -50 - Math.random() * 200, y: Math.random() * (H + 600), speed: 0.3 + Math.random() * 0.5, phase: Math.random() * Math.PI * 2, life: 500 + Math.random() * 300, z: 20, parallax: 0.35 }); }
                    if (dt === 'balloons') { for (var i = 0; i < 5; i++) { var r = 40 + Math.random() * 90; _bgElements.push({ type: 'balloon', x: Math.random() * W, y: Math.random() * (H + 600), r: r, color: 'hsl(' + Math.floor(Math.random() * 360) + ',80%,60%)', speed: 0.05 + Math.random() * 0.15, life: 800 + Math.random() * 400, z: 10 + r/100, parallax: 0.2 + r/500 }); } }
                    if (dt === 'stars') { for (var i = 0; i < 20; i++) _bgElements.push({ type: 'star', x: Math.random() * W, y: Math.random() * (H + 600), size: 2.5 + Math.random() * 12.5, twinkle: Math.random() * Math.PI * 2, life: 900 + Math.random() * 300, z: 1, parallax: 0.01 }); }
                    if (dt === 'comets') { for (var i = 0; i < 1; i++) _bgElements.push({ type: 'comet', x: -50, y: Math.random() * (H + 600), speed: 2 + Math.random() * 3, angle: 0.3 + Math.random() * 0.4, life: 200 + Math.random() * 100, z: 2, parallax: 0.02 }); }
                    if (dt === 'planets') { _bgElements.push({ type: 'planet', x: W * 0.2 + Math.random() * W * 0.6, y: Math.random() * (H + 600), r: 75 + Math.random() * 125, color: 'hsl(' + Math.floor(Math.random() * 360) + ',40%,50%)', life: 900, z: 0, parallax: 0.05 }); }
                    if (dt === 'snow') { for (var i = 0; i < 20; i++) _bgElements.push({ type: 'snow', x: Math.random() * W, y: Math.random() * (H + 600), vx: (Math.random() - 0.5) * 0.8, vy: 0.4 + Math.random() * 1.2, size: 7.5 + Math.random() * 15, life: 500 + Math.random() * 300, z: 30, parallax: 0.4 }); }
                    if (dt === 'rainbow') { _bgElements.push({ type: 'rainbow', life: 800, y: 0, z: 3, parallax: 0.05 }); }
                    if (dt === 'aurora') { for (var i = 0; i < 2; i++) _bgElements.push({ type: 'aurora', idx: i, life: 900, y: 0, z: 0, parallax: 0.02 }); }
                }
            }
            ...
            for (var ti = 0; ti < types.length; ti++) {
                var dt = types[ti];
                if (dt === 'clouds' && (counts.cloud || 0) < 3) { var cw = 200 + Math.random() * 500; _bgElements.push({ type: 'cloud', x: -100 - Math.random() * 100, y: Math.random() * (H + 600), w: cw, speed: 0.1 + Math.random() * 0.25, life: 600 + Math.random() * 400, z: 4 + cw/1000, parallax: 0.1 + cw/2000 }); }
                if (dt === 'birds' && (counts.bird || 0) < 3) _bgElements.push({ type: 'bird', x: -50 - Math.random() * 100, y: Math.random() * (H + 600), speed: 0.3 + Math.random() * 0.5, phase: Math.random() * Math.PI * 2, life: 500 + Math.random() * 300, z: 20, parallax: 0.35 });
                if (dt === 'balloons' && (counts.balloon || 0) < 4) { var r = 40 + Math.random() * 90; _bgElements.push({ type: 'balloon', x: Math.random() * W, y: Math.random() * (H + 600), r: r, color: 'hsl(' + Math.floor(Math.random() * 360) + ',80%,60%)', speed: 0.05 + Math.random() * 0.15, life: 800 + Math.random() * 400, z: 10 + r/100, parallax: 0.2 + r/500 }); }
                if (dt === 'stars' && (counts.star || 0) < 15) _bgElements.push({ type: 'star', x: Math.random() * W, y: Math.random() * (H + 600), size: 2.5 + Math.random() * 12.5, twinkle: Math.random() * Math.PI * 2, life: 900 + Math.random() * 300, z: 1, parallax: 0.01 });
                if (dt === 'snow' && (counts.snow || 0) < 15) _bgElements.push({ type: 'snow', x: Math.random() * W, y: Math.random() * (H + 600), vx: (Math.random() - 0.5) * 0.8, vy: 0.4 + Math.random() * 1.2, size: 7.5 + Math.random() * 15, life: 500 + Math.random() * 300, z: 30, parallax: 0.4 });
                if (dt === 'comets' && (counts.comet || 0) < 1 && Math.random() < 0.005) _bgElements.push({ type: 'comet', x: -50, y: Math.random() * (H + 600), speed: 2 + Math.random() * 3, angle: 0.3 + Math.random() * 0.4, life: 200 + Math.random() * 100, z: 2, parallax: 0.02 });
                if (dt === 'aurora' && (counts.aurora || 0) < 2) _bgElements.push({ type: 'aurora', idx: Math.floor(Math.random() * 4), life: 900, y: 0, z: 0, parallax: 0.02 });
                if (dt === 'rainbow' && (counts.rainbow || 0) < 1) _bgElements.push({ type: 'rainbow', life: 800, y: 0, z: 3, parallax: 0.05 });
            }
            for (var i = _bgElements.length - 1; i >= 0; i--) {
                var e = _bgElements[i]; e.life--;
                if (e.life <= 0) { _bgElements.splice(i, 1); continue; }
                if (e.type === 'cloud') { e.x += e.speed; if (e.x > W + 200) e.x = -e.w - 100; }
                else if (e.type === 'bird') { e.x += e.speed; e.phase += 0.1; if (e.x > W + 100) e.x = -80; }
                else if (e.type === 'balloon') { e.y -= e.speed; e.x += Math.sin(Date.now() * 0.002 + i) * 0.2; }
                else if (e.type === 'star') { e.twinkle += 0.04; }
                else if (e.type === 'comet') { e.x += e.speed; e.y += e.speed * e.angle; if (e.x > W + 50) { e.x = -50; e.y = Math.random() * (H + 600); } }
                else if (e.type === 'snow') { e.x += e.vx; e.y += e.vy; }
            }
        }
        ...
        function drawWeatherOverlay() {
            ...
            // Ordenar por Z-index para manter a hierarquia certa (planetas no fundo, nuvens grandes na frente de menores, etc)
            _bgElements.sort(function(a, b) { return (a.z || 0) - (b.z || 0); });
            
            for (var i = 0; i < _bgElements.length; i++) {
                var e = _bgElements[i];
                // Suavizando sumiço: tempo de fade in/out aumentado para 120 frames (2 segundos)
                var fi = Math.min(1, (800 - e.life) / 120), fo = Math.min(1, e.life / 120), al = Math.min(fi, fo);
                
                // Cálculo de Parallax com Wrapping vertical
                var wrapWin = H + 1000;
                var parallaxY = e.y - (typeof camY !== 'undefined' ? camY : 0) * (e.parallax || 0);
                var py = ((parallaxY % wrapWin) + wrapWin) % wrapWin - 300;
                
                if (e.type === 'cloud') { 
                    ctx.globalAlpha = 0.85 * al; 
                    var cloudsArr = ['mc_cloud_1', 'mc_cloud_2', 'mc_cloud_3', 'mc_cloud_4'];
                    var tkey = cloudsArr[Math.floor(e.w) % cloudsArr.length];
                    if (BG_TEX[tkey] && BG_TEX[tkey].complete && BG_TEX[tkey].naturalWidth > 0) {
                        ctx.drawImage(BG_TEX[tkey], e.x, py - e.w*0.3, e.w, e.w * 0.6);
                    }
                }
                else if (e.type === 'bird') { 
                    ctx.globalAlpha = 0.5 * al; ctx.strokeStyle = '#222'; ctx.lineWidth = 2.5; 
                    var wy = Math.sin(e.phase) * 6; 
                    ctx.beginPath(); ctx.moveTo(e.x - 30, py + wy); ctx.quadraticCurveTo(e.x - 10, py - 20 + wy, e.x, py); ctx.quadraticCurveTo(e.x + 10, py - 20 + wy, e.x + 30, py + wy); ctx.stroke(); 
                }
                else if (e.type === 'balloon') { 
                    ctx.globalAlpha = Math.min(1, 1.2 * al); 
                    if (BG_TEX['mc_balloon'] && BG_TEX['mc_balloon'].complete && BG_TEX['mc_balloon'].naturalWidth > 0) {
                        var br = e.r * 2.5; 
                        ctx.drawImage(BG_TEX['mc_balloon'], e.x - br/2, py - br/2, br, br);
                    }
                }
                else if (e.type === 'star') { 
                    var tw = 0.2 + Math.sin(e.twinkle) * 0.3; 
                    ctx.globalAlpha = tw * al; 
                    ctx.shadowBlur = 10; ctx.shadowColor = '#fff'; ctx.fillStyle = '#fff';
                    ctx.beginPath(); ctx.arc(e.x, py, e.size, 0, Math.PI * 2); ctx.fill(); 
                    if (e.size > 1.5 && tw > 0.35) { ctx.strokeStyle = 'rgba(255,255,255,' + (tw * 0.5) + ')'; ctx.lineWidth = 0.5; ctx.beginPath(); ctx.moveTo(e.x - e.size * 2, py); ctx.lineTo(e.x + e.size * 2, py); ctx.stroke(); ctx.beginPath(); ctx.moveTo(e.x, py - e.size * 2); ctx.lineTo(e.x, py + e.size * 2); ctx.stroke(); } 
                    ctx.shadowBlur = 0;
                }
                else if (e.type === 'comet') {
                    ctx.globalAlpha = 0.8 * al; ctx.fillStyle = '#fff'; 
                    ctx.shadowBlur = 15; ctx.shadowColor = '#0ff';
                    ctx.beginPath(); ctx.arc(e.x, py, 8, 0, Math.PI * 2); ctx.fill();
                    ctx.shadowBlur = 0;
                }
                else if (e.type === 'planet') { 
                    ctx.globalAlpha = Math.min(1, 1 * al); 
                    var tkey = Math.floor(e.r) % 2 === 0 ? 'mc_planet' : 'mc_moon';
                    if (BG_TEX[tkey] && BG_TEX[tkey].complete && BG_TEX[tkey].naturalWidth > 0) {
                        var pr = e.r * 3.5; 
                        ctx.drawImage(BG_TEX[tkey], e.x - pr/2, py - pr/2, pr, pr);
                    }
                }
                else if (e.type === 'snow') { 
                    ctx.globalAlpha = 0.9 * al; ctx.fillStyle = '#fff'; 
                    ...
                    if (BG_TEX['mc_rainbow'] && BG_TEX['mc_rainbow'].complete && BG_TEX['mc_rainbow'].naturalWidth > 0) {
                        var rw = W * 0.8;
                        var rh = rw * (BG_TEX['mc_rainbow'].height / BG_TEX['mc_rainbow'].width);
                        var rx = W * 0.1;
                        var ry = py + H * 0.4 - rh/2;
                        ctx.globalCompositeOperation = 'source-over';
                        ctx.drawImage(BG_TEX['mc_rainbow'], rx, ry, rw, rh);
                        ctx.globalAlpha = 0.8 * al; ctx.fillStyle = '#fff';
                        var cx1 = rx + rw * 0.05, cy1 = ry + rh * 0.95, cw1 = 150;
                        ctx.beginPath(); ctx.arc(cx1, cy1, cw1*0.3, 0, Math.PI*2); ctx.fill(); ctx.beginPath(); ctx.arc(cx1+cw1*0.22, cy1-cw1*0.12, cw1*0.25, 0, Math.PI*2); ctx.fill(); ctx.beginPath(); ctx.arc(cx1-cw1*0.18, cy1+cw1*0.05, cw1*0.22, 0, Math.PI*2); ctx.fill();
                        var cx2 = rx + rw * 0.95, cy2 = ry + rh * 0.95, cw2 = 150;
                        ctx.beginPath(); ctx.arc(cx2, cy2, cw2*0.3, 0, Math.PI*2); ctx.fill(); ctx.beginPath(); ctx.arc(cx2-cw2*0.22, cy2-cw2*0.12, cw2*0.25, 0, Math.PI*2); ctx.fill(); ctx.beginPath(); ctx.arc(cx2+cw2*0.18, cy2+cw2*0.05, cw2*0.22, 0, Math.PI*2); ctx.fill();
                    }
                }
                else if (e.type === 'aurora') { ctx.globalAlpha = 0.15 * al; var ay = py + H * 0.06 + e.idx * 22 + Math.sin(t + e.idx) * 12; var ag = ctx.createLinearGradient(0, ay, W, ay); ag.addColorStop(0, '#00ff88'); ag.addColorStop(0.3, '#00ffcc'); ag.addColorStop(0.6, '#8844ff'); ag.addColorStop(1, '#00ff88'); ctx.fillStyle = ag; ctx.fillRect(0, ay, W, 10 + Math.sin(t * 2 + e.idx) * 5); }
            }
        ...
        function drawPickaxeInfo(p) {
            ...
            var AVATAR_SCALE = 1.3;
            var avSize = Math.round((p.isCompanion ? 32 : 48) * AVATAR_SCALE);
            var avY = p.y - PICK_HALF - Math.round((p.isCompanion ? 34 : 44) * AVATAR_SCALE);
        ...
        function draw() {
            ...
            for (var r = rEnd; r >= rStart; r--) {
                for (var c = 0; c < COLS; c++) {
                    ...
                    if (tex3d) {
                        var depth = Math.floor(TILE * 0.3);
                        ctx.drawImage(tex3d, x, y - depth, tex3d.width, tex3d.height);
                        var gTex = BGLOW_TEX[cell.t];
                        if (gTex && gTex.complete && gTex.naturalWidth > 0) {
                            ctx.save();
                            ctx.globalCompositeOperation = 'lighter';
                            ctx.globalAlpha = 0.6 + Math.sin(Date.now() * 0.005 + x * 0.1 + y * 0.1) * 0.4;
                            ctx.drawImage(gTex, x, y, TILE, TILE);
                            ctx.restore();
                        }
                    } else if (tex && tex.complete && tex.naturalWidth > 0) {
                        ...
                        if (gTex && gTex.complete && gTex.naturalWidth > 0) {
                            ...
                            ctx.globalCompositeOperation = 'lighter';
                            ctx.globalAlpha = 0.6 + Math.sin(Date.now() * 0.005 + x * 0.1 + y * 0.1) * 0.4;
                            ctx.drawImage(gTex, x - 0.5, y - 0.5, TILE + 1, TILE + 1);
                            ctx.restore();
                        }
                    } else {
            ...
            for (var d = 0; d < debris.length; d++) {
                ...
                if (tex && tex.complete && tex.naturalWidth > 0) {
                    ...
                    var gTex = BGLOW_TEX[db.t];
                    if (gTex && gTex.complete && gTex.naturalWidth > 0) {
                        ctx.save();
                        ctx.globalCompositeOperation = 'lighter';
                        ctx.globalAlpha = 0.6 + Math.sin(Date.now() * 0.005 + db.x) * 0.4;
                        ctx.drawImage(gTex, -db.size / 2, -db.size / 2, db.size, db.size);
                        ctx.restore();
                    }
                } else {
            ...
            // ── Nuke & Thor visuals ──
            if (_nukeActive) {
                var nSize = (45 - _nukeTimer) * 50 * (1 + _nukeLevel * 0.1);
                ctx.save();
            ...
            for (var tk = 0; tk < _thorBeams.length; tk++) {
                ...
                var lvl = getPickaxeLevel(tb.player);
                var mult = 1 + lvl * 0.1;
                var pColor = (tb.player.pickaxe && tb.player.pickaxe.color) ? tb.player.pickaxe.color : (tb.player.color || '#00ffff');

                if (SHADOW_ENABLED) { ctx.shadowBlur = 15 * mult; ctx.shadowColor = pColor; }
                ctx.strokeStyle = pColor; ctx.lineWidth = 6 * mult; ctx.stroke();

                if (SHADOW_ENABLED) { ctx.shadowBlur = 25 * mult; ctx.shadowColor = '#ffffff'; }
                ctx.strokeStyle = '#ffffff'; ctx.lineWidth = 2 * mult; ctx.stroke();
                ctx.restore();
            }
            ctx.globalAlpha = 1;
            ...
            for (var ei = 0; ei < extraPicks.length; ei++) {
                ...
                var scale = (ep.bigTimer > 0) ? (BIG_SCALE * (1 + getPickaxeLevel(ep) * 0.1)) : 1;
                if (scale !== 1) ctx.scale(scale, scale);
        ...
        var frameMinDelay = 1000 / 60; // 16.6667ms per frame (60fps)
        ...
        function loop() {
            ...
            if (elapsed < frameMinDelay) {
                return;
            }

            // Adjust lastFrameTime to be the theoretical frame time to avoid accumulation drift
            lastFrameTime = now - (elapsed % frameMinDelay);
            ...
            // ── Otimização: Processa eventos da fila com delay para evitar queda de FPS ──
            var nowTime = Date.now();
            if (liveEventQueue.length > 0 && nowTime - _lastEventProcessTime > 100) { // 100ms delay
                processLiveEvent(liveEventQueue.shift());
                _lastEventProcessTime = nowTime;
            }
        }
        ...
        function drawEnemyHearts(x, y, hp, maxHp, isBoss) {
...
// Damage player if their material is inferior
        ...
        function drawBats() {
            ...
            for (var i = 0; i < bats.length; i++) {
                ...
                // Smooth direction flipping to avoid flickering
        ...
        function sfxSheep(force) {
            ...
            activeSheepSounds = activeSheepSounds.filter(function (s) { return !s.ended; });
        ...
        function sfxPig(force) {
            ...
            if (!force) {
                var visible = bonusBalls.some(function (b) {
                    return b.animalType === 'pig' && b.y > camY && b.y < canvas.height + camY;
                });
                if (!visible) return;
            }

            activePigSounds = activePigSounds.filter(function (s) { return !s.ended; });
        ...
        window.updateUIMode = function () {
            // Tudo agora fica visível no adminPanel, não precisamos mais alternar elementos individuais
        ...
        function processLiveEvent(data) {
            ...
            // Para presentes do TikTok, incluímos o repeatCount no ID para permitir combos
            if (data.type === 'gift' && data.msgId) {
                eventId = data.msgId + '_' + (data.repeatCount || 1);
            }
            ...
            if (data.type === 'connected') {
                ...
                words.forEach(function (word) {
                    ...
                    if (cmd === 'play' || cmd === 'join') { /* já foi spawnado acima */ }
                    ...
                    else if (cmd === 'ball' || cmd === 'bola') activateBall(tx, ty, cUser, cAvatar);
                        ...
                        if (!(skillCooldowns[cdKey] > 0)) {
                            ...
                            // Gera 5 creepers ao redor do jogador
                ...
                if (repeat === 1) {
                    ...
                    if (repeat <= lastRep) {
                        incrementalRepeat = 0; // Evita duplicar se recebermos um repeat menor ou igual ao já processado
                    } else {
                ...
                if (giftName.includes('perfume')) {
                    ...
                    var oldInv = getPlayerInventory(gUser);
                    var oldPick = oldInv[0] || PICKAXES[0];
                    ...
                    var newPick = inv[0] || PICKAXES[0];
                    
                    var isMainUpgrade = (newPick.name !== oldPick.name);
                    var isAdditionalUpgrade = false;
                    var upgradedSlotIndex = -1;
                    
                    if (!isMainUpgrade) {
                        for (var k = 1; k < Math.max(oldInv.length, inv.length); k++) {
                            var oldSlot = oldInv[k];
                            var newSlot = inv[k];
                            if (!oldSlot && newSlot) {
                                isAdditionalUpgrade = true;
                                upgradedSlotIndex = k;
                                break;
                            } else if (oldSlot && newSlot && oldSlot.name !== newSlot.name) {
                                isAdditionalUpgrade = true;
                                upgradedSlotIndex = k;
                                break;
                            }
                    ...
                    if (isMainUpgrade || isAdditionalUpgrade) {
                        showPickaxeUpgradeOverlay(gUser, gAvatar, oldPick, newPick, isAdditionalUpgrade, upgradedSlotIndex, inv);
                    }
                }
                ...
                else if (giftName.includes('donut') || giftName.includes('rosquinha') || giftName.includes('doughnut')) {
                ...
                else if (giftName.includes('urso misha') || giftName.includes('misha bear') || giftName.includes('mishka bear') || giftName.includes('🧸')) {
                    for (var ri = 0; ri < incrementalRepeat; ri++) {
                        activateDiamondSwords(gUser);
                    }
                }
                else if (giftName.includes('heart with hands') || giftName.includes('coração com mão') || giftName.includes('hand hearts')) {
                    for (var ri = 0; ri < incrementalRepeat; ri++) {
                    ...
                    spawnText(gx, gy - 30, '❤️ ' + count + ' HEARTS!', '#ff6688'); if (pick.active) score += 3 * incrementalRepeat;
                }
                    ...
                    if (targetPlayer) {
                        ...
                        var statusMsg = targetPlayer.isChicken ? '🐔 CHICKEN MODE ON!' : '👨‍🌾 CHICKEN MODE OFF!';
                ...
                else if (giftName.includes('presente popular') || giftName.includes('super popular') || giftName.includes('vote')) {
                    var targetPlayer = gPick || (gUser === (pick.userName || '').toLowerCase() ? pick : null);
                    if (targetPlayer) {
                        ...
                        var statusMsg = targetPlayer.isSteve ? '💎 STEVE TIME!' : '⛏ NORMAL MODE!';
                    ...
                    if (targetPlayer) {
                        ...
                        var statusMsg = targetPlayer.isHomer ? '🍩 HOMER TIME!' : '⛏ NORMAL MODE!';
                ...
                if (Math.floor(newLikes / 1000) > Math.floor(oldLikes / 1000)) {
                    var oldPick = getPickaxeForGifts(lUser);
                    ...
                    // Exibe animação de upgrade de picareta sem fundo
                    if (newPick && oldPick && newPick.name !== oldPick.name) {
                        showPickaxeUpgradeOverlay(lUser, lAvatar, oldPick, newPick);
                    }
                }
                ...
                if (!sPick) {
                    spawnUserPickaxe(sUser, data.avatar || '');
                    sPick = userPicks.find(function (up) { return up.userName === sUser; });
                }
                ...
                if (!isMegaSpamming(sUser)) {
                    activateMegaTNT(sx, sy, sUser, data.avatar || '', true);
                    if (pick.active) score += 10;
                    spawnText(sx, sy - 30, '+10 SHARE!', '#44ddff');
                }
            }
        }
        ...
        function spawnClone(userName, avatarUrl) {
            ...
            for (var i = 0; i < numClones; i++) {
                extraPicks.push({
                    ...
                    swordTimer: sourcePick.swordTimer || 0,
                    bigTimer: sourcePick.bigTimer || 0
                });
            }
        ...
        function activateNuke(tx, ty, userName) {
            ...
            var pLvl = sourcePlayer ? getPickaxeLevel(sourcePlayer) : 0;
        ...
        function activateBlackHole(tx, ty, userName) {
            ...
            var pLvl = getPickaxeLevel(sourcePlayer);

            skillCooldowns[cdKey] = blackholeCooldown;
            activeBlackHoles.push({ id: Math.random(), x: bx, y: by, timer: BLACKHOLE_DURATION, userName: userName, level: pLvl });
        ...
        function updateBlackHoles() {
            for (var i = activeBlackHoles.length - 1; i >= 0; i--) {
                ...
                var bhLvl = bh.level || 0;
                var R = BLACKHOLE_RADIUS * (1 + bhLvl * 0.1);
                ...
                function applyBHSuck(list, devourDist) {
                    var scaledDevour = devourDist * (1 + bhLvl * 0.1);
                    for (var j = list.length - 1; j >= 0; j--) {
                        ...
                        if (dist < R || obj._bhCapturedBy === bh.id) {
                            ...
                            var nx = dx / (dist || 1), ny = dy / (dist || 1);
                            var targetSpeed = (1.5 + closeness * 4) * (1 + bhLvl * 0.1);
                        ...
                        for (var r = cr - tileR; r <= cr + tileR; r++) {
                            for (var c = cc - tileR; c <= cc + tileR; c++) {
                                ...
                                if (blockDist < destroyR) {
                                    var dmg = Math.max(1, Math.floor(3 * (1 - blockDist / destroyR) * (1 + bhLvl * 0.1)));
                                    cell.hp -= dmg;
        ...
        function drawBlackHoles() {
            for (var i = 0; i < activeBlackHoles.length; i++) {
                ...
                var R = BLACKHOLE_RADIUS * (1 + (bh.level || 0) * 0.1);
                if (bh._collapsing) {
        ...
        function activateThor(tx, ty, userName) {
            ...
            if (inWater && submergedEnemies.length > 0) {
                ...
                for (var si = 0; si < submergedEnemies.length; si++) {
                    ...
                    var dmg = Math.ceil((isZombie ? 3 : 2) * (1 + getPickaxeLevel(sourcePlayer) * 0.1));
            ...
            if (hasTarget) {
                var lvl = getPickaxeLevel(sourcePlayer);
                ...
                spawnText(startX, startY - 40, '⚡⚡ THOR ' + (10 + lvl) + 'x!', '#00ffff');
            }
        ...
        function spawnRoulette(userName) {
            ...
            var prizes = [
                ...
                { name: 'perfume', color: 'rgba(255, 105, 180, 0.6)', text: '🧴', imgId: 'imgGiftPerfume' }
            ];
            ...
            function drawWheel() {
                ctx.clearRect(0, 0, 117, 117);
                var arc = Math.PI * 2 / prizes.length;

                ctx.save();
                ctx.translate(58.5, 58.5);
                ctx.rotate(angle);
                ...
                for (var i = 0; i < prizes.length; i++) {
                    ...
                    if (prizes[i].imgId) {
                        ...
                        if (img && img.src && img.style.display !== 'none') {
                            ...
                            ctx.textAlign = 'center';
                            ctx.textBaseline = 'middle';
                            ctx.fillText(prizes[i].text, 36, 0);
                        }
                    } else {
                        // Text for 2x
                        ctx.fillStyle = '#000';
                ...
                if (ps && ps.avatar) {
                    ...
                    if (avatarImg && avatarImg.complete && avatarImg.naturalWidth > 0) {
                        ...
                        ctx.clip();
                        ctx.drawImage(avatarImg, 37.5, 37.5, 42, 42);
                        ctx.restore();
                ...
                // Outer border
                ctx.beginPath();
                ctx.arc(58.5, 58.5, 56, 0, Math.PI * 2);
                ctx.lineWidth = 2;
                ctx.strokeStyle = '#fff';
                ctx.stroke();
            }
        ...
        function applyPrize(userName, prizeName) {
            ...
            if (prizeName === '2x') {
                ...
                } else if (prizeName === 'perfume') {
                    spawnText(tx, ty - 30, '🧴 Ganhou Perfume!', '#ff44ff');
                    processLiveEvent({ type: 'gift', user: userName, avatar: avatar, giftName: 'perfume', diamondCount: 1, repeatCount: 1, msgId: 'roulette_' + Date.now(), isRoulette: true });
                }
            }
        }
        ...
        function updateLeaderboard() {
            ...
            try {
                ...
                var playersInGame = allSorted
                    ...
                    .map(function (p) {
                        ...
                        if (ownerName && ownerName.toLowerCase() === nameLower) {
                            pTier = getPickaxeForGifts(ownerName);
                ...
                playersInGame.forEach(function (u, i) {
                    ...
                    html += '    <div style="text-align:left; min-width:65px;">';
        ...
        function showGiftAlert(userName, userAvatar, giftName, giftImgUrl, rewardInfo) {
            ...
            if (!container) {
                ...
                document.getElementById('gameWrapper').appendChild(container);
            }
            ...
            var giftLabel = giftName.toUpperCase();
            if (giftLabel.includes('FATIA DE BOLO') || giftLabel.includes('CAKE SLICE')) giftLabel = 'CAKE SLICE';
            else if (giftLabel.includes('PRESENTE POPULAR') || giftLabel.includes('SUPER POPULAR')) giftLabel = 'SUPER POPULAR GIFT';
            else if (giftLabel.includes('CORACAO COM MAO') || giftLabel.includes('CORAÇÃO COM MÃO') || giftLabel.includes('HAND HEARTS')) giftLabel = 'HAND HEARTS';
        ...
        // ── UI de Alerta de Upgrade de Picareta (Overlay Sem Fundo) ─────────────────
        function showPickaxeUpgradeOverlay(userName, userAvatar, oldPick, newPick, isAdditional, upgradedSlotIndex, fullInventory) {
            var wrapper = document.createElement('div');
            wrapper.className = 'pickaxe-upgrade-wrapper';
            
            // Determine highlight color
            var highlightColor = newPick.color || '#ffd700';
            if (isAdditional && fullInventory && fullInventory[upgradedSlotIndex]) {
                highlightColor = fullInventory[upgradedSlotIndex].color || '#ffd700';
            }

            // Glow aura
            if (!isAdditional) {
                var aura = document.createElement('div');
                aura.className = 'upgrade-glow-aura';
                aura.style.background = 'radial-gradient(circle, ' + highlightColor + '44 0%, rgba(0,0,0,0) 70%)';
                wrapper.appendChild(aura);
            }

            // Content box
            var contentBox = document.createElement('div');
            contentBox.className = 'upgrade-content-box';

            // User Avatar
            var avatarWrapper = document.createElement('div');
            avatarWrapper.className = 'upgrade-avatar-glow';
            
            var avatarImg = document.createElement('img');
            avatarImg.className = 'upgrade-avatar-img';
            var defAvatar = 'data:image/svg+xml;utf8,<svg xmlns="http://www.w3.org/2000/svg" width="40" height="40" viewBox="0 0 24 24" fill="%23ccc"><path d="M12 12c2.21 0 4-1.79 4-4s-1.79-4-4-4-4 1.79-4 4 1.79 4 4 4zm0 2c-2.67 0-8 1.34-8 4v2h16v-2c0-2.66-5.33-4-8-4z"/></svg>';
            avatarImg.src = getProxyUrl(userAvatar) || defAvatar;
            avatarImg.style.boxShadow = '0 0 25px ' + highlightColor + ', inset 0 0 10px rgba(0,0,0,0.5)';
            avatarImg.style.borderColor = highlightColor;
            avatarWrapper.appendChild(avatarImg);
            contentBox.appendChild(avatarWrapper);

            // User name text
            var userText = document.createElement('div');
            userText.className = 'upgrade-user-text';
            userText.innerText = (userName.charAt(0) === '@' ? userName : '@' + userName).toUpperCase();
            contentBox.appendChild(userText);

            // Pickaxe container
            var pickaxeContainer = document.createElement('div');
            pickaxeContainer.className = 'upgrade-pickaxes-container';

            if (!isAdditional) {
                // Classic main pickaxe upgrade
                // Old pickaxe img
                var oldPickImg = document.createElement('img');
                oldPickImg.className = 'upgrade-pickaxe-old-anim';
                oldPickImg.src = oldPick.img;
                pickaxeContainer.appendChild(oldPickImg);

                // New pickaxe img
                var newPickImg = document.createElement('img');
                newPickImg.className = 'upgrade-pickaxe-new-anim';
                newPickImg.src = newPick.img;
                if (newPick.color) {
                    newPickImg.style.color = newPick.color;
                }
                pickaxeContainer.appendChild(newPickImg);
            } else {
                // Additional pickaxe unlock/evolution!
                // 1. Draw main maxed pickaxe in center
                var centerPickImg = document.createElement('img');
                centerPickImg.className = 'upgrade-pickaxe-new-anim';
                centerPickImg.style.position = 'absolute';
                centerPickImg.style.width = '100px';
                centerPickImg.style.height = '100px';
                centerPickImg.style.objectFit = 'contain';
                centerPickImg.style.zIndex = '5';
                centerPickImg.src = newPick.img;
                centerPickImg.style.transform = 'scale(0) rotate(-360deg)';
                centerPickImg.style.opacity = '0';
                centerPickImg.style.animation = 'pickaxeNewSpin 0.8s cubic-bezier(0.175, 0.885, 0.32, 1.275) 0.2s forwards';
                if (newPick.color) {
                    centerPickImg.style.color = newPick.color;
                }
                
                // Once spun in, switch to pulse
                (function(imgEl) {
                    setTimeout(function() {
                        imgEl.style.transform = 'scale(1) rotate(0deg)';
                        imgEl.style.opacity = '1';
                        imgEl.style.animation = 'avatarPulse 2s infinite ease-in-out';
                    }, 200 + 800);
                })(centerPickImg);
                
                pickaxeContainer.appendChild(centerPickImg);

                // 2. Draw additional pickaxes orbiting sequentially
                var orbitRadius = 65;
                for (var k = 1; k < fullInventory.length; k++) {
                    var addPick = fullInventory[k];
                    var angle = ((k - 1) / 4) * Math.PI * 2 - Math.PI / 2; // Up to 4 positions around the center
                    var cx = Math.cos(angle) * orbitRadius;
                    var cy = Math.sin(angle) * orbitRadius;
                    
                    var delaySec = 0.3 + (k - 1) * 0.4;

                    if (k === upgradedSlotIndex) {
                        // Find old state of this companion pickaxe to spin out
                        var companionNewTier = PICKAXES.findIndex(p => p.name === addPick.name);
                        var companionOldTier = Math.max(0, companionNewTier - 1);
                        var oldCompanionPick = PICKAXES[companionOldTier];

                        // Old companion pickaxe (spins out)
                        var oldCompanionImg = document.createElement('img');
                        oldCompanionImg.className = 'upgrade-pickaxe-old-anim';
                        oldCompanionImg.style.position = 'absolute';
                        oldCompanionImg.style.width = '42px';
                        oldCompanionImg.style.height = '42px';
                        oldCompanionImg.style.objectFit = 'contain';
                        oldCompanionImg.style.left = 'calc(50% + ' + cx + 'px - 21px)';
                        oldCompanionImg.style.top = 'calc(50% + ' + cy + 'px - 21px)';
                        oldCompanionImg.src = oldCompanionPick.img;
                        oldCompanionImg.style.zIndex = '9';
                        oldCompanionImg.style.opacity = '0';
                        oldCompanionImg.style.transform = 'scale(0) rotate(-360deg)';
                        oldCompanionImg.style.animation = 'pickaxeNewSpin 0.5s cubic-bezier(0.175, 0.885, 0.32, 1.275) ' + delaySec + 's forwards';
                        if (oldCompanionPick.color) {
                            oldCompanionImg.style.color = oldCompanionPick.color;
                        }
                        
                        // We spin it out right after showing it briefly
                        (function(imgEl, startDelay) {
                            setTimeout(function() {
                                imgEl.style.animation = 'pickaxeOldSpin 0.5s cubic-bezier(0.6, -0.28, 0.735, 0.045) forwards';
                            }, startDelay * 1000 + 400);
                        })(oldCompanionImg, delaySec);

                        pickaxeContainer.appendChild(oldCompanionImg);

                        // New companion pickaxe (spins in)
                        var newCompanionImg = document.createElement('img');
                        newCompanionImg.className = 'upgrade-pickaxe-new-anim';
                        newCompanionImg.style.position = 'absolute';
                        newCompanionImg.style.width = '42px';
                        newCompanionImg.style.height = '42px';
                        newCompanionImg.style.objectFit = 'contain';
                        newCompanionImg.style.left = 'calc(50% + ' + cx + 'px - 21px)';
                        newCompanionImg.style.top = 'calc(50% + ' + cy + 'px - 21px)';
                        newCompanionImg.src = addPick.img;
                        newCompanionImg.style.zIndex = '10';
                        newCompanionImg.style.transform = 'scale(0) rotate(-360deg)';
                        newCompanionImg.style.opacity = '0';
                        newCompanionImg.style.animation = 'pickaxeNewSpin 0.8s cubic-bezier(0.175, 0.885, 0.32, 1.275) ' + (delaySec + 0.5) + 's forwards';
                        if (addPick.color) {
                            newCompanionImg.style.color = addPick.color;
                        }
                        pickaxeContainer.appendChild(newCompanionImg);
                    } else {
                        // Static/glowing companion (spins in sequentially too)
                        var companionImg = document.createElement('img');
                        companionImg.className = 'upgrade-pickaxe-new-anim';
                        companionImg.style.position = 'absolute';
                        companionImg.style.width = '42px';
                        companionImg.style.height = '42px';
                        companionImg.style.objectFit = 'contain';
                        companionImg.style.left = 'calc(50% + ' + cx + 'px - 21px)';
                        companionImg.style.top = 'calc(50% + ' + cy + 'px - 21px)';
                        companionImg.src = addPick.img;
                        companionImg.style.zIndex = '4';
                        companionImg.style.transform = 'scale(0) rotate(-360deg)';
                        companionImg.style.opacity = '0';
                        companionImg.style.animation = 'pickaxeNewSpin 0.8s cubic-bezier(0.175, 0.885, 0.32, 1.275) ' + delaySec + 's forwards';
                        if (addPick.color) {
                            companionImg.style.color = addPick.color;
                        }
                        ...
                        (function(imgEl, startDelay) {
                            setTimeout(function() {
                                imgEl.style.transform = 'scale(1) rotate(0deg)';
                                imgEl.style.opacity = '1';
                                imgEl.style.animation = 'avatarPulse 3s infinite ease-in-out';
                            }, startDelay * 1000 + 800);
                        })(companionImg, delaySec);

                        pickaxeContainer.appendChild(companionImg);
                    }
                }
            }

            contentBox.appendChild(pickaxeContainer);

            // Title text
            var titleText = document.createElement('div');
            titleText.className = 'upgrade-title-text';
            if (isAdditional) {
                titleText.innerText = 'NOVA PICARETA AUXILIAR!';
            } else {
                titleText.innerText = 'UPGRADE DE PICARETA!';
            }
            titleText.style.color = highlightColor;
            contentBox.appendChild(titleText);

            wrapper.appendChild(contentBox);
            (document.getElementById('gameWrapper') || document.body).appendChild(wrapper);

            // Play first sound at the start
            if (_audioStarted) {
                sfxPickaxeChange();
            }

            // Play staggered entry sound effects for each slot
            if (isAdditional) {
                for (var k = 1; k < fullInventory.length; k++) {
                    (function(slotIdx) {
                        var spinStartDelay = (0.3 + (slotIdx - 1) * 0.4) * 1000;
                        setTimeout(function() {
                            if (_audioStarted) {
                                sfxPickaxeChange();
                            }
            ...
            // Play main powerup sound and show burst particles when upgraded slot pops in
            var mainEventDelay = isAdditional ? (0.3 + (upgradedSlotIndex - 1) * 0.4 + 0.5) * 1000 : 600;
            setTimeout(function() {
                if (_audioStarted) {
                    playPowerUp();
                }
                ...
                for (var i = 0; i < 20; i++) {
                    createUpgradeParticle(wrapper, highlightColor);
                }
            }, mainEventDelay);

            // Remove overlay after standard delay (extended if sequential additional picks)
            var totalDuration = isAdditional ? (3000 + (fullInventory.length - 1) * 400) : 3000;
            setTimeout(function() {
        ...
        function createUpgradeParticle(parent, color) {
            ...
            function update() {
                ...
                if (progress >= 1) {
                    ...
                    p.style.left = 'calc(50% + ' + (vx * progress) + 'px)';
                    p.style.top = 'calc(50% + ' + (vy * progress) + 'px)';
                    p.style.opacity = 1 - progress;
                    p.style.transform = 'translate(-50%, -50%) scale(' + (1 - progress * 0.5) + ')';
        ...
        // ── UI de Alerta de Espada Encontrada (Overlay Sem Fundo) ───────────────────
        function showSwordFoundOverlay(userName, userAvatar, swordType, swordColor) {
            var wrapper = document.createElement('div');
            wrapper.className = 'sword-found-wrapper';
            
            // Glow aura
            var aura = document.createElement('div');
            aura.className = 'sword-glow-aura';
            if (swordColor) {
                aura.style.background = 'radial-gradient(circle, ' + swordColor + '44 0%, rgba(0,0,0,0) 70%)';
            }
            wrapper.appendChild(aura);

            // Content box
            var contentBox = document.createElement('div');
            contentBox.className = 'sword-content-box';

            // User Avatar
            var avatarWrapper = document.createElement('div');
            avatarWrapper.className = 'sword-avatar-glow';
            
            var avatarImg = document.createElement('img');
            avatarImg.className = 'sword-avatar-img';
            var defAvatar = 'data:image/svg+xml;utf8,<svg xmlns="http://www.w3.org/2000/svg" width="40" height="40" viewBox="0 0 24 24" fill="%23ccc"><path d="M12 12c2.21 0 4-1.79 4-4s-1.79-4-4-4-4 1.79-4 4 1.79 4 4 4zm0 2c-2.67 0-8 1.34-8 4v2h16v-2c0-2.66-5.33-4-8-4z"/></svg>';
            avatarImg.src = getProxyUrl(userAvatar) || defAvatar;
            if (swordColor) {
                avatarImg.style.boxShadow = '0 0 20px ' + swordColor + ', inset 0 0 10px rgba(0,0,0,0.5)';
                avatarImg.style.borderColor = swordColor;
            }
            avatarWrapper.appendChild(avatarImg);
            contentBox.appendChild(avatarWrapper);
            ...
            // User name text
            var userText = document.createElement('div');
            userText.className = 'sword-user-text';
            userText.innerText = (userName.charAt(0) === '@' ? userName : '@' + userName).toUpperCase();
            contentBox.appendChild(userText);

            // Sword image container
            var swordContainer = document.createElement('div');
            swordContainer.className = 'sword-image-container';

            // Sword img
            var swordImg = document.createElement('img');
            swordImg.className = 'sword-img-anim';
            var sAsset = SWORD_ASSETS[swordType];
            swordImg.src = sAsset ? sAsset.src : 'sword/Wooden_Sword.png';
            if (swordColor) {
                swordImg.style.color = swordColor;
            }
            swordContainer.appendChild(swordImg);
            contentBox.appendChild(swordContainer);

            // Title text
            var titleText = document.createElement('div');
            titleText.className = 'sword-title-text';
            titleText.innerText = 'ESPADA DE ' + translateSwordType(swordType) + '!';
            if (swordColor) {
                titleText.style.color = swordColor;
            }
            contentBox.appendChild(titleText);
            ...
            setTimeout(function() {
                for (var i = 0; i < 20; i++) {
                    createSwordParticle(wrapper, swordColor || '#ff4444');
                }
            }, 100);
        ...
        function translateSwordType(type) {
            var trans = {
                'Wood': 'MADEIRA',
                'Stone': 'PEDRA',
                'Iron': 'FERRO',
            ...
            return trans[type] || type.toUpperCase();
        }

        function createSwordParticle(parent, color) {
            var p = document.createElement('div');
            p.className = 'sword-particle';
            p.style.background = color;
            
            // Random direction
            var angle = Math.random() * Math.PI * 2;
            var velocity = 40 + Math.random() * 120;
            var vx = Math.cos(angle) * velocity;
            var vy = Math.sin(angle) * velocity;
        ...
        async function generateScoreReport() {
            ...
            // 1. Gather players data
            var hostUser = ownerName || ('@' + (localStorage.getItem('pd_tiktok_username') || '').toLowerCase());
            var hAvUrl = (persistentScores[hostUser] && persistentScores[hostUser].avatar) ? persistentScores[hostUser].avatar : 'tiktok.png';
            ...
            // Helper to resolve avatar/image with CORS & cache support
            async function resolveAvatarImage(url) {
                const rawUrl = url || 'tiktok.png';
            ...
            // Draw grid pattern for tech aesthetic
            ...
            // Draw scattered background decorations (creeper, tnt, diamond, etc.)
            const decoPaths = ['block/creeper.png', 'block/tnt.png', 'item/diamond.png', 'pickaxe/diamond_pickaxe.png', 'slime/Slime.png'];
            for (var i = 0; i < 20; i++) {
                var dPath = decoPaths[Math.floor(Math.random() * decoPaths.length)];
                var dImg = await resolveAvatarImage(dPath);
                if (dImg) {
                    var tx = Math.random() * W;
                    var ty = Math.random() * H;
                    var ds = 80 + Math.random() * 120;
                    var dr = Math.random() * Math.PI * 2;
                    var dop = 0.04 + Math.random() * 0.06;

                    x.save();
                    x.globalAlpha = dop;
                    x.translate(tx, ty);
                    x.rotate(dr);
                    x.drawImage(dImg, -ds/2, -ds/2, ds, ds);
                    x.restore();
                }
            }
            ...
            x.textAlign = 'center';
            x.fillStyle = '#00ff88';
            x.font = '900 90px Inter, Arial';
            x.shadowColor = '#00ff88';
            x.shadowBlur = 20;
            x.fillText('TOP RANKING', W / 2, 160);

            x.shadowBlur = 0;
            x.font = '800 32px Inter, Arial';
            x.fillStyle = 'rgba(255,255,255,0.5)';
            x.fillText('PICKAXE DROP • ' + dateStr, W / 2, 225);

            // Draw Divider
            x.strokeStyle = 'rgba(255,255,255,0.1)';
            x.lineWidth = 2;
            x.beginPath(); x.moveTo(100, 260); x.lineTo(W - 100, 260); x.stroke();

            // Draw Leaderboard Rows (Top 10 players)
            var startY = 290;
            var rowH = 120;
            var rowGap = 16;
            ...
            for (var i = 0; i < players.length; i++) {
                var p = players[i];
                var curY = startY + i * (rowH + rowGap);
                var medal = (i === 0) ? 'gold' : (i === 1 ? 'silver' : (i === 2 ? 'bronze' : ''));

                // Row background container
                x.fillStyle = 'rgba(255,255,255,0.04)';
                x.beginPath(); x.roundRect(100, curY, W - 200, rowH, 25); x.fill();
                x.strokeStyle = 'rgba(255,255,255,0.08)';
                x.lineWidth = 1;
                x.beginPath(); x.roundRect(100, curY, W - 200, rowH, 25); x.stroke();
                ...
                if (medal === 'gold') {
                    ...
                    x.font = '900 36px Inter, Arial';
                    x.fillText('#' + (i + 1), 150, curY + 60);
                }

                // Avatar
                var avImg = await resolveAvatarImage(p.avatar);
                var avSize = 80;
                var avX = 200, avY = curY + 20;
                x.save();
                x.beginPath(); x.roundRect(avX, avY, avSize, avSize, 18); x.clip();
                if (avImg) {
                    x.drawImage(avImg, avX, avY, avSize, avSize);
                } else {
                    x.fillStyle = '#111'; x.fillRect(avX, avY, avSize, avSize);
                    x.fillStyle = '#fff'; x.font = '36px Arial'; x.textAlign = 'center'; x.fillText('👤', avX + avSize/2, avY + avSize/2 + 12);
                }
                x.restore();

                x.strokeStyle = 'rgba(255,255,255,0.2)';
                x.lineWidth = 2;
                x.beginPath(); x.roundRect(avX, avY, avSize, avSize, 18); x.stroke();

                // Player Pickaxe (in front of avatar)
                var inv = getPlayerInventory(p.name);
                var playerPick = inv[0] || PICKAXES[0];
                var pickImg = await resolveAvatarImage(playerPick.img);
                if (pickImg) {
                    x.save();
                    if (playerPick.color) {
                        x.shadowColor = playerPick.color;
                        x.shadowBlur = 12;
                    }
                    x.drawImage(pickImg, 305, curY + 28, 64, 64);
                    x.restore();
                }
                ...
                x.fillStyle = '#ffffff';
                x.font = '900 36px Inter, Arial';
                var dispName = p.name.replace(/^@/, '');
                x.fillText(dispName, 390, curY + 40);

                // Stats
                var statsText = [];
                if (p.likes > 0) statsText.push('❤️ ' + p.likes);
                if (p.gifts > 0) statsText.push('🎁 ' + p.gifts);
                var statsTextStr = statsText.length > 0 ? statsText.join('   ') : 'PARTICIPANTE';
                x.fillStyle = '#00ff88';
                x.font = '800 22px Inter, Arial';
                x.fillText(statsTextStr, 390, curY + 80);

                // Draw additional pickaxes as small icons inline
                var additionalPicks = inv.slice(1);
                if (additionalPicks.length > 0) {
                    var statsWidth = x.measureText(statsTextStr).width;
                    var nextX = 390 + statsWidth + 30;
                    for (var j = 0; j < additionalPicks.length; j++) {
                        var addPick = additionalPicks[j];
                        var addPickImg = await resolveAvatarImage(addPick.img);
                        if (addPickImg) {
                            x.save();
                            if (addPick.color) {
                                x.shadowColor = addPick.color;
                                x.shadowBlur = 8;
                            }
                            x.drawImage(addPickImg, nextX, curY + 80 - 18, 36, 36);
                            x.restore();
                            nextX += 36 + 8;
                        }
                    }
                ...
                x.textAlign = 'right';
                x.fillStyle = '#ffd700';
                x.font = '900 40px Inter, Arial';
                x.fillText(Math.floor(p.score) + '', W - 140, curY + 60);
            }
            ...
            x.strokeStyle = 'rgba(255,255,255,0.1)';
            x.lineWidth = 2;
            x.beginPath(); x.moveTo(100, footY - 40); x.lineTo(W - 100, footY - 40); x.stroke();

            // Draw Thank You card
            x.fillStyle = 'rgba(0, 255, 136, 0.03)';
            x.beginPath(); x.roundRect(100, footY - 20, W - 200, 160, 30); x.fill();
            x.strokeStyle = 'rgba(0, 255, 136, 0.15)';
            x.lineWidth = 2;
            x.beginPath(); x.roundRect(100, footY - 20, W - 200, 160, 30); x.stroke();

            // Host Avatar inside thank you card
            var hostAvImg = await resolveAvatarImage(hAvUrl);
            var hAvSize = 110;
            var hAvX = 140, hAvY = footY + 5;
            x.save();
            x.beginPath(); x.arc(hAvX + hAvSize/2, hAvY + hAvSize/2, hAvSize/2, 0, Math.PI * 2); x.clip();
            if (hostAvImg) {
                x.drawImage(hostAvImg, hAvX, hAvY, hAvSize, hAvSize);
            } else {
                x.fillStyle = '#111'; x.fillRect(hAvX, hAvY, hAvSize, hAvSize);
            }
            x.restore();

            // Host avatar border and glow
            x.save();
            x.strokeStyle = '#00ff88';
            x.lineWidth = 4;
            x.shadowColor = '#00ff88';
            x.shadowBlur = 15;
            x.beginPath(); x.arc(hAvX + hAvSize/2, hAvY + hAvSize/2, hAvSize/2, 0, Math.PI * 2); x.stroke();
            x.restore();
            ...
            x.fillStyle = '#ffffff';
            x.font = '900 36px Inter, Arial';
            x.fillText('THANK YOU ALL!', hAvX + hAvSize + 40, footY + 45);

            x.fillStyle = 'rgba(255,255,255,0.7)';
            x.font = '700 24px Inter, Arial';
            x.fillText('Thanks for participating in our stream!', hAvX + hAvSize + 40, footY + 85);
            x.fillText('Follow us for the next match!', hAvX + hAvSize + 40, footY + 115);

            // Sub-footer text
            x.textAlign = 'center';
            x.fillStyle = 'rgba(255,255,255,0.15)';
            x.font = '900 18px Inter, Arial';
            x.fillText('GENERATED BY PICKAXE DROP SERVER', W / 2, H - 20);

            // 8. Auto-save to Server & Local Download
            try {
                const dataUrl = c.toDataURL('image/png');
                const fname = 'ranking_geral_' + now.getTime() + '.png';

                // Local download (triggers browser download dialog)
                ...
                // Server-side save to Desktop
                saveRankingToServer(dataUrl, fname);

                spawnText(canvas.width / 2, camY + canvas.height / 2, '✅ REPORT GENERATED & SAVED!', '#00ff88');
            } catch (e) {
        ...
        async function generateCommemorativeImage() {
            ...
            try {
                ...
                // Removido o download local para evitar prompts no navegador
        ...
        function applyGiftImages(gifts) {
            ...
            gifts.forEach(function (g) {
                ...
                if (g.name.includes('urso misha') || g.name.includes('misha bear') || g.name.includes('mishka bear')) {
                    var el = document.getElementById('imgGiftMisha');
                    var txt = document.getElementById('txtGiftMisha');
                    if (el && txt) { el.src = getProxyUrl(g.url); el.style.display = 'inline-block'; txt.style.display = 'none'; }
                }
                if (g.name.includes('heart with hands') || g.name.includes('coração com mão') || g.name.includes('hand hearts')) {
                ...
                if (g.name.includes('donut') || g.name.includes('rosquinha') || g.name.includes('doughnut')) {
